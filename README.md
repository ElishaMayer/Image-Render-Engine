# Image Render Engine
As part of the course "Introduction to Software Engineering" we wrote from scratch a image render engine in Java.
...
4. Once it finished rendering, the picture will be in /images.
<br>
At the end of the page is a guide how to write the XML.

# How it Works
In order to render a picture you need to define a scene. <br>Basically, a scene is a camera in 3D with one or more of the following objects:
 Plane, Triangle, Sphere, Tube, Cylinder, Square and Cube. <br>You can also add one of the following lights: Ambient Light, Directional Light, Point Light and Spotlight.
...
2. We added Bounding Volume Hierarchy. We build a tree by putting each group of geometries in a AABB box, then all the boxes are grouped again recursively until there is only one box.
<br>
...
First the xml root need to look like the following example:
```xml
...
## Add lights
The color is represented in RGB format.
<br>
<br>
To add Ambient light
```xml
<ambient-light color="50 50 50" ka="0.5"/>
```
Where Ka is a factor of the brightness (A positive number).
<br>
<br>
...
```
Where Kc Kl Kq are attenuation factors.
<br>
<br>
...
### Sphere
```xml
<sphere center="0 0 -50" radius="50" emission="255 0 0" material="0.5 0.5 10"/>
```
### Triangle
```xml
<triangle p0="100 0 -49" p1="0 100 -49" p2="100 100 -49" emission="255 0 0" material="0.5 0.5 10"/>
```
### Plane
```xml
<plane p="-100 0 -49" v="0 -100 -49" emission="255 0 0" material="0.5 0.5 10"/>
```
### Tube
```xml
<tube Ray-p="-100 0 -49" Ray-v="0 -100 -49" radius="50" emission="255 0 0" material="0.5 0.5 10"/>
```
### Cylinder
1. Add by middle point and height:
```xml
<cylinder Ray-p="0 0 0" Ray-v="0 1 0" radius="50" heigth="30" emission="255 0 0" material="0.5 0.5 10"/>
```
2. Add by Two points:
```xml
<cylinder2p p1="-250 220 -800" p2="250 220 -800" radius="30" emission="0 0 20" material="0.5 0.5 10"/>
```
### Square
```xml
<square p0="10 10 0" p1="-10 10 0" p2="-10 -10 0" p3="10 -10 0" emission="50 100 50" material="0.5 0.5 10"/>
```
Where the four points each one is next to the other.

### Cube
```xml
<cube p0="10 10 0" p1="-10 10 0" p2="-10 -10 0" p3="10 -10 0" bp="10 10 -10" height="20" emission="0 20 20" material="0.5 0.5 10"/>
```
...
1. By Using only 3 parameters. Defusal (Kd), Specular (Ks) and Shininess (ShN).
```material="0.5 0.5 100"```
2. By using the same 3 parameters and another 2, Transparency (Kt) and Reflection (Kr).
...
## Emission
Each geometry has a value to set the color:
```
emisson=100 100 100
```
The color is represented in RGB format.
